				...
				if empty_node:
					if '>' in sent[-2]:
						...
						for index,temp_dep in enumerate(empty_node_deps):
							...
							headid = dep.split(':')[0]
							rels = ':'.join(dep.split(':')[1:])
							temp_sent.append([sent[0]+'.'+str(index+1),'_','X','X','X','X=X',headid,rels,temp_dep,'X'])
				if '.' in sent[0]:
					total_add_token+=1
					addition_flag=1
					addbuf.append(sent[0])
...
for file in files:
	...
	# print(file,end=' ')
	print(file)
	if 'conllu' in filedir:
		back_conversion(filedir,write=True,empty_node=True,extra_name='_back')	
